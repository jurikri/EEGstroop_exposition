...
def stroop_tscore(weights = [[0.5, 0.5], [0.2, 0.3, 0.5]]):
    # if False:
    ...
    import numpy as np
    import pandas as pd        
    df2 = pd.read_excel('features_output.xlsx')
    df3 = np.array(df2)
    relative_score = []
    for c in range(3,9):
        if c in [3,4,5]:
            tmp = (1 / (np.argsort(np.array(df3[:,c], dtype=float)) + 1))
        elif c in [6,7,8]:
            tmp = (1 / (np.argsort(np.array(df3[:,c], dtype=float))[::-1] + 1))
        
        relative_score.append(tmp)
    relative_score = np.transpose(np.array(relative_score)) # n * fn
    
    df2.iloc[:,2] = np.mean(relative_score, axis=1)
    df2.to_excel('features_output.xlsx')
    
    # print(np.mean(relative_score, axis=1))
    return None
